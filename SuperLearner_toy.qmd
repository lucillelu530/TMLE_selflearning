...
## Parallel Processing in sl3

Using `future` Package, dont need to modify any code in order switch from sequential on the local machine to distributed processing on a remote compute cluster. And global variables and functions are automatically identified and exported as needed, making it straightforward to tweak existing code to make use of futures.”
...
lrn_glm<-Lrnr_glm$new() #Lrnr_glm is a R6class, and lrnr_glm. is a new object which contain several properties
...
#Put all learners together so SL could consider them as candicates
stack <- Stack$new(
  lrn_glm, lrn_mean, lrn_ridge, lrn_lasso, lrn_polspline, lrn_earth, lrn_hal, 
  lrn_ranger, lrn_xgb, lrn_gam, lrn_bayesglm
)
#Stack is also a class(or say, a learner itself) and we create a stack object using a template

#INstantiate the SL with stacked learners and metalearner
...
runtime_sl_fit_parallel #161.61s
```

## Tuning Parameters in SL

### Define your own model configurations
...
### Automatically grid and selection with Caret
...
Review: How to fit a SL?

-   Define the prediction task with `make_sl3_task`

-   Instantiate the SL with `Lrnr_sl`

-   Fit the SL to the task with `train`
...
First do cross_validation to fit the metalearner.

```{r}
#Rolling origin:
folds = origami::make_folds(tsdata, fold_fun=folds_rolling_origin, first_window = 10, validation_size = 10, gap = 0, batch = 1)

#We can also specify how far in the future we want to train our model on.And increase the training size in each fold
...
### Defining `sl3` learners

`Lrnr_base` : a base class for defining machine learning algorithms, as well as fits for those algorithms to particular `sl3_task` . Different machine learning algorithms are defined in classes that inherit from `Lrnr_base`. All learners (e.g., `Lrnr_arima`) inherit from this R6 class.

Therefore, "learners" in `sl3` refer to the family of classes that inherit from `Lrnr_base` . A `learner` **Object** can be constructed from their class definitions using the `make_learner` function (or just use `Lrnr_xxx$new()` method)
...
All learners define three main methods: `train`, `predict`, and `chain`. The first, `train`, takes an `sl3_task` object, and returns a `learner_fit`, which has the same class as the learner that was trained:
...
ts_stack_preds <- ts_stack_fit$predict() #prediction on validation set
head(ts_stack_preds)
```

### Cross-Validation in model fitting

```{r cv_ts_example}
#| message: false
#| warning: false
cv_ts_stack <- Lrnr_cv$new(ts_stack) #Fit/Predict a learner with Cross Validation
cv_ts_fit <- cv_ts_stack$train(ts_uni_task) #train on the whole dataset
...
ts_ml_fit <- metalearner$train(cv_ts_uni_task) #metalearner is trained on validation set
```
